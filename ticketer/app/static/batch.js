// The batch screen: one batch's drafts, reviewed and sent to 311.
...
import { getMode, modalController } from "/vendor/ionic-9.0.4/index.esm.js";
import { api, whoami } from "./api.js";
import { back } from "./nav.js";
import { EXPIRY_WARN_MS, confirmed, expiryTag, fmtDateTime, fmtLeft, fmtTime, notice, plural, showError, tag } from "./ui.js";
...
const backButton = () => (getMode() === "ios"
  ? html`<ion-button @click=${() => back("/")}><ion-icon slot="start" name="chevron-back"></ion-icon>Batches</ion-button>`
  : html`<ion-button aria-label="Back to batches" @click=${() => back("/")}><ion-icon slot="icon-only" name="arrow-back"></ion-icon></ion-button>`);
...
function reviewSummary(batch, canReview) {
  ...
  // Once anything has been sent, what happened to it is the news, not the review tally: that
  // would go on saying "1 to report" about a request 311 already has.
...
function addressFact(capture, draft) {
  ...
  let note;
  if ("address" in edits) note = tag("edited", "info");
...
// lit-html draws the page, so a redraw updates it in place rather than rebuilding it: a rebuilt
// Ionic component stays invisible until it has booted, and the page would blink on every refresh
// while extraction runs. For the same reason `class` on an Ionic element is only ever bound
// through classMap -- a plain binding would wipe the classes Ionic adds, `hydrated` among them,
// and the element would disappear.
class BatchPage extends HTMLElement {
  // batchId is set by the router, from the address.
  connectedCallback() {
    this.shown = null; // { batch, me } on screen
    this.failure = null; // why the last load failed
    this.timer = null;
    this.saving = null; // capture id with a review change in flight
    this.submitting = false; // a submit request is in flight
    this.editor = null; // { id, values, plateChecked, reportAfter, root, modal } while the edit sheet is open
    this.payloads = new Map(); // submission id -> the raw request as text, or null while it loads
    this.draw();
    this.load();
  }
  ...
  async load() {
    clearTimeout(this.timer);
    try {
      const [batch, me] = await Promise.all([api("GET", `/api/batches/${encodeURIComponent(this.batchId)}`), whoami()]);
      ...
      if (batch.status === "queued" || batch.status === "processing" || working) {
        this.timer = setTimeout(() => this.load(), BATCH_REFRESH_MS);
      }
    } catch (error) {
      if (!this.isConnected) return;
  ...
  draw() {
    ...
    if (this.editor) render(this.editorView(), this.editor.root);
  }

  view() {
    const batch = this.shown?.batch;
  ...
  content() {
    const problem = this.failure ? notice("error", `Couldn't load this batch: ${this.failure}`,
      html`<div><ion-button fill="outline" size="small" @click=${() => this.load()}>Try again</ion-button></div>`) : nothing;
    if (!this.shown) return this.failure ? problem : html`<div class="loading"><ion-spinner></ion-spinner></div>`;

    const { batch, me } = this.shown;
    ...
    return html`
      ...
      ${drafts.error ? notice("error", `${plural(drafts.error, "photo")} couldn't be extracted.`,
        html`<div><ion-button fill="outline" size="small" color="danger" @click=${() => this.retryExtraction()}>Retry failed</ion-button></div>`) : nothing}
      ${batch.captures.map((capture, index) => this.draftCard(capture, index, canReview))}
      ${batch.status === "ready" && canReview ? this.submitPanel(batch, me) : nothing}
      <div class="end-actions">
        ${batch.status === "ready" && drafts.done && canReview
          ? html`<ion-button fill="clear" @click=${() => this.rerun()}>
        ...
        ${batch.status !== "capturing" && me.user_login === batch.created_by
          ? html`<ion-button fill="outline" color="danger" @click=${() => this.deleteBatch()}>
  ...
  draftCard(capture, index, canReview) {
    const decision = capture.draft?.review.decision;
    return html`
      <ion-card class=${classMap({ draft: true, report: decision === "report", skip: decision === "skip" })}>
        <div class="photo">
        ...
        ${this.draftDetails(capture, index, canReview)}
      </ion-card>`;
  }

  draftDetails(capture, index, canReview) {
    const { batch } = this.shown;
    const draft = capture.draft;
    ...
    const { values, edits } = draft.review;
    const flags = batch.status === "ready" ? draftFlags(capture, batch) : [];
    let plateTag = nothing;
    if (edited(edits, "plate_text", "plate_state")) plateTag = tag("edited", "info");
    ...
    if (edited(edits, "color", "make", "model")) vehicleTag = tag("edited", "info");
    ...
    return html`
      <ion-card-header>
        ...
        <div class="plate-row">
          ...
          ${canReview ? html`<ion-button class="edit" fill="clear" size="small" ?disabled=${this.saving != null}
            @click=${() => this.openEditor(capture)}><ion-icon slot="start" name="create-outline"></ion-icon>Edit</ion-button>` : nothing}
        </div>
      </ion-card-header>
      ...
      ${capture.submission ? this.submissionBlock(capture.submission) : nothing}
      ${canReview ? this.reviewControls(capture) : nothing}`;
  }
  ...
  submissionBlock(submission) {
    return html`
      <div class="submission">
        ...
        ${submission.description ? this.sentDetail(submission) : nothing}
  ...
  sentDetail(submission) {
    const payload = this.payloads.get(submission.id);
    return html`
      <ion-accordion-group>
        <ion-accordion value="sent">
          ...
          <div slot="content" class="sent-detail">
            ...
            ${payload != null
              ...
              : html`<ion-button fill="outline" size="small" ?disabled=${this.payloads.has(submission.id)}
                  @click=${() => this.showPayload(submission)}>Show the raw request</ion-button>`}
          </div>
        </ion-accordion>
      </ion-accordion-group>`;
  }

  async showPayload(submission) {
    this.payloads.set(submission.id, null);
    this.draw();
    try {
      const full = await api("GET", `/api/batches/${encodeURIComponent(this.batchId)}/submissions/${submission.id}`);
      this.payloads.set(submission.id, JSON.stringify(full.case_record, null, 1));
    } catch (error) {
      this.payloads.delete(submission.id);
  ...
  reviewControls(capture) {
    const { decision } = capture.draft.review;
    const option = (value, label, color) => html`
      <ion-button fill=${decision === value ? "solid" : "outline"} color=${color} ?disabled=${this.saving != null}
        aria-pressed=${String(decision === value)}
        @click=${() => this.decide(capture, decision === value ? null : value)}>
  ...
  decide(capture, decision) {
    ...
    if (decision === "report" && (plate_needs_check || missingFields(values).length)) {
      return this.openEditor(capture, { reportAfter: true });
    }
    return this.saveReview(capture, { decision });
  }

  async saveReview(capture, changes) {
    const { batch } = this.shown;
    this.saving = capture.id;
    this.draw();
    try {
  ...
  async openEditor(capture, { reportAfter = false } = {}) {
    if (this.editor || this.saving) return;
    const root = document.createElement("div");
    const editor = { id: capture.id, values: { ...capture.draft.review.values }, plateChecked: false, reportAfter, root, modal: null };
    this.editor = editor;
    render(this.editorView(), root);
    // Presented over this page, so on iOS the page shrinks back behind it like a card.
    const modal = await modalController.create({ component: root, presentingElement: this });
    editor.modal = modal;
  ...
  editingCapture() {
    return this.shown?.batch.captures.find((c) => c.id === this.editor?.id);
  }

  editorView() {
    const capture = this.editingCapture();
    ...
    const busy = this.saving != null;
    const saveLabel = this.saving === capture.id ? "Saving…" : form.reportAfter ? "Save & report" : "Save";
    const set = (field) => (event) => {
      form.values[field] = event.target.value ?? "";
      if (form.reportAfter) render(this.editorView(), form.root); // keep "To report, fill in" current
    };
      ...
      <ion-header>
        <ion-toolbar>
          ...
          <ion-buttons slot="end">
            <ion-button strong ?disabled=${busy} @click=${() => this.saveEdits()}>${form.reportAfter ? "Report" : "Save"}</ion-button>
          </ion-buttons>
        </ion-toolbar>
      </ion-header>
      <ion-content>
        ...
        ${offerReadings ? html`<div class="readings muted">Use:
          ${readings.map(([who, text]) => html`<ion-chip aria-label="Use ${who} reading ${text}"
            @click=${() => { form.values.plate_text = text; render(this.editorView(), form.root); }}>
        ...
        <div class="edit-actions">
          <ion-button expand="block" ?disabled=${busy} @click=${() => this.saveEdits()}>${saveLabel}</ion-button>
        </div>
      </ion-content>`;
  }
  ...
  async saveEdits() {
    const form = this.editor;
    const capture = this.editingCapture();
    if (!form || !capture || this.saving) return;
      ...
      <ion-card class="send">
        <ion-card-content>
          ...
          <div class="small">${dryRun
            // The switch lives in the app's own options, which can't be reached from here -- so
          ...
          <ion-button expand="block" fill=${dryRun ? "outline" : "solid"} ?disabled=${this.submitting}
            @click=${() => this.submitDrafts(batch, sendable, dryRun)}>
            <ion-icon slot="start" name="paper-plane"></ion-icon>
            ${this.submitting ? "Sending…" : dryRun ? `Dry run ${plural(sendable.length, "request")}` : `Send ${plural(sendable.length, "request")} to 311`}
          </ion-button>
        </ion-card-content>
      </ion-card>`;
  }
  ...
  async retryExtraction(rerunAll = false) {
    try {
      await api("POST", `/api/batches/${encodeURIComponent(this.batchId)}/retry${rerunAll ? "?rerun_all=true" : ""}`);
    } catch (error) {
      return showError(`Couldn't retry: ${error.message}`);
    }
    this.querySelector("ion-content")?.scrollToTop(300);
    this.load();
  }
  ...
  async rerun() {
    ...
    })) this.retryExtraction(true);
  }
}

customElements.define("page-batch", BatchPage);
